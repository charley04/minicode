...
import type { AppConfig, AgentEvent, Session, ApprovalDecision, ContextInfo } from "./types.js";
import { renderToolCallInline, renderToolResult, renderApprovalPreview, renderTaskSummary } from "./renderers.js";
...
async function runOneShot(config: AppConfig, useSkills: boolean, useMcp: boolean, prompt: string) {
 ...
 console.log(renderToolResult(event.name, event.display ?? event.output, event.error === true, " ", event.durationMs));
 ...
 const summary = renderTaskSummary(agent.getToolCallLog());
...
async function runInteractive(config: AppConfig, useSkills: boolean, useMcp: boolean, resumeId?: string) {
 ...
 const preview = lines[0].slice(0, 30).trimEnd();
 const suffix = lines.length > 1 ? ` +${lines.length - 1}L` : "";
 const token = `[📋 ${preview}${suffix}]`;
 ...
 // Fallback for terminals that do not support bracketed paste mode
 // (e.g. legacy Windows CMD). If a chunk contains a newline and non-whitespace
 // text, it is almost certainly a clipboard paste rather than a single
 // Enter key. Wrap it as a single token so the first newline is not executed.
 const isEnterOnly = s.length <= 2 && s.trim().length === 0;
 if (!pasting && s.includes("\n") && !isEnterOnly) {
 const payload = s.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
 ...
 console.log(buildStatusBar(config, { skills, mcp, messages: session.messages }));
 rl.prompt();

 process.stdin.on("keypress", (char: string, key: { name?: string; ctrl?: boolean; meta?: boolean; sequence?: string } | undefined) => {
 // Get current line from readline for hint synchronization
 const currentInput = rl.line || "";

 if (!isProcessing) {
 handleKeyPress(char, key, currentInput);
 return;
 }
 ...
 // Echo the user's message with full multi-line formatting before agent runs.
 console.log();
 renderUserMessageBlock(trimmed);
 ...
 console.log(chalk.bold.cyan("\n Resuming Task\n"));
 console.log(chalk.gray(" ────────────────────────────────────────"));
 console.log(` Interrupted at: ${new Date(cp.interruptedAt).toLocaleString()}`);
 console.log(` Turns completed: ${chalk.yellow(String(cp.completedTurns))}`);
 console.log(` Original input: "${chalk.white(cp.userInput.slice(0, 60))}${cp.userInput.length > 60 ? '...' : ''}"`);
 console.log(chalk.gray("\n ⏳ Continuing from breakpoint...\n"));

 // Set up for resume - the agent will use startTurn
 isProcessing = true;
 interruptRequested = false;
 activeController = new AbortController();

 const resumeTools = new ToolRegistry();
 if (mcp) resumeTools.registerMany(mcp.getTools());

 const resumeSpinner = new Spinner("Resuming...");
 let resumeSpinnerActive = false;
 let resumeTextBuffer = "";
 let resumeHasBufferedText = false;
 ...
 startTurn: cp.completedTurns, // Start from where we left off
 onEvent: (event) => {
 if (event.type === "text") {
 if (!resumeHasBufferedText) {
 resumeHasBufferedText = true;
 if (resumeSpinnerActive) { resumeSpinner.stop(); resumeSpinnerActive = false; }
 }
 resumeTextBuffer += event.content;
 } else if (event.type === "tool_call") {
 if (resumeHasBufferedText && resumeTextBuffer.trim()) { stdout.write(renderAssistantBlock(resumeTextBuffer)); resumeTextBuffer = ""; resumeHasBufferedText = false; }
 else if (resumeHasBufferedText) { stdout.write(resumeTextBuffer); resumeTextBuffer = ""; resumeHasBufferedText = false; }
 if (resumeSpinnerActive) { resumeSpinner.stop(); resumeSpinnerActive = false; }
 stdout.write("\n");
 console.log(chalk.yellow(" ⚡ ") + chalk.bold(event.name) + " " + renderToolCallInline(event.name, event.args));
 resumeSpinner.update(`Running ${event.name}...`);
 resumeSpinner.start();
 resumeSpinnerActive = true;
 } else if (event.type === "tool_result") {
 if (resumeSpinnerActive) { resumeSpinner.stop(); resumeSpinnerActive = false; }
 console.log(renderToolResult(event.name, event.display ?? event.output, event.error === true, " ", event.durationMs));
 resumeSpinner.update("Thinking...");
 resumeSpinner.start();
 resumeSpinnerActive = true;
 } else if (event.type === "todo_update") {
 progress.update(event.todos);
 ...
 if (resumeSpinnerActive) { resumeSpinner.stop(); resumeSpinnerActive = false; }
 rl.pause();
 try { return await promptApproval(name, args); } finally { rl.resume(); }
 },
 });
 ...
 try {
 const updated = await resumeAgent.run(session.messages);
 session.messages = updated;
 // Clear checkpoint after successful resume
 if ((updated as any)._checkpoint?.isInterrupted) {
 session.checkpoint = (updated as any)._checkpoint;
 } else {
 ...
 console.log(chalk.yellow(`\n ⏸ Interrupted again. You can use /continue to resume once more.`));
 // Update checkpoint with new state
 session.checkpoint = {
 ...session.checkpoint!,
 interruptedAt: Date.now(),
 completedTurns: (session.checkpoint?.completedTurns || 0), // Will be updated by agent
 isInterrupted: true,
 };
 saveSession(session);
 } else {
 ...
 const summary = renderTaskSummary(resumeAgent.getToolCallLog());
 if (summary) console.log(summary);
 ...
 : "";
 const ctxTag = lastContextInfo
 ? chalk.gray(` · ${lastContextInfo.usagePercent}% ctx`)
 : "";
 spinner.update(base + tokenTag + ctxTag);
 };
 ...
 // Check if agent returned checkpoint data (interrupted)
 if ((updated as any)._checkpoint?.isInterrupted) {
 session.checkpoint = {
 ...(updated as any)._checkpoint,
 userInput: trimmed,
 };
 console.log(chalk.yellow(`\n ⏸ Interrupted at turn ${(updated as any)._checkpoint.completedTurns}. Use /continue to resume.`));
 } else {
 // Task completed successfully - clear any old checkpoint
 session.checkpoint = undefined;
 }
...
function buildStatusBar(
 config: AppConfig,
 ctx: { skills?: SkillManager; mcp?: MCPManager; contextInfo?: ContextInfo | null; messages?: any[] },
): string {
 ...
 const skillCount = ctx.skills?.getAll().length || 0;
 if (skillCount > 0) {
 segments.push(`🧩 ${skillCount} skills`);
 }

 const mcpToolCount = ctx.mcp?.getTools().length || 0;
 if (mcpToolCount > 0) {
 segments.push(`🔌 ${mcpToolCount} mcp`);
 }

 // Show context usage info if available
 if (ctx.contextInfo) {
 const pct = ctx.contextInfo.usagePercent;
 const color = pct > 90 ? chalk.red : pct > 70 ? chalk.yellow : chalk.green;
 segments.push(color(`📊 ${pct}% ctx`));
 } else if (ctx.messages && ctx.messages.length > 0) {
 // Calculate and show context usage on the fly
 const msgTokens = estimateMessagesTokens(ctx.messages);
 const maxCtx = config.maxContextTokens || 200000;
 const pct = Math.round((msgTokens / maxCtx) * 100);
 const color = pct > 90 ? chalk.red : pct > 70 ? chalk.yellow : chalk.green;
 const bar = renderContextBar(Math.min(pct, 100));
 segments.push(color(`${bar} ${formatTokens(msgTokens)}t`));
 }

 if (config.autoApprove) {
 segments.push(chalk.red("⚠ auto"));
 }
 if (config.sandbox) {
 segments.push(chalk.yellow("🐳 sandbox"));
 }

 const content = segments.join(" · ");
 const visibleLen = stripAnsiVisibleLen(content);
 const padding = Math.max(0, inner - visibleLen);
 return chalk.cyan(" ╭─ ") + content + " ".repeat(padding) + chalk.cyan(" ╮");
}

function buildPrompt(_config: AppConfig): string {
 return ` ${chalk.cyan("│")} ${chalk.green.bold("❯ ")}`;
}
 ...
 */
function renderUserMessageBlock(content: string): void {
 ...
 if (lineCount > 1) {
 console.log(header + chalk.gray(` (${lineCount} lines)`));
 } else {
 console.log(header);
 }

 // Top border
 console.log(chalk.gray(" ┌" + "─".repeat(innerWidth + 2) + "┐"));
 ...
 console.log(railLeft + rawLine + " ".repeat(Math.max(0, innerWidth - stripAnsi(rawLine).length)) + railRight);
 } else {
 ...
 console.log(railLeft + segment + (isLast ? " ".repeat(Math.max(0, innerWidth - stripAnsi(segment).length)) : "") + railRight);
 }
 }
 }
...
function wrapLine(line: string, maxWidth: number): string[] {
 ...
 let lastSpace = searchStr.lastIndexOf(" ");
...
async function handleSlashCommand(
 ...
 const originalTokens = estimateMessagesTokens(session.messages);
 const maxTarget = Math.floor((config.maxContextTokens || 200000) * 0.6); // Target 60% of max

 if (originalTokens < maxTarget) {
 console.log(chalk.gray(` Context is already optimized (~${formatTokens(originalTokens)} tokens). No compression needed.`));
 break;
 }

 const result = compressContext(session.messages, maxTarget);
 session.messages = result.compressed;
 saveSession(session);

 console.log(chalk.bold.cyan("\n Context Compression\n"));
 console.log(chalk.gray(" ────────────────────────────────────────"));
 console.log(` Original:  ${chalk.yellow(formatTokens(result.originalTokens) + " tokens")}`);
 console.log(` Compressed: ${chalk.green(formatTokens(result.newTokens) + " tokens")}`);
 console.log(` Removed:   ${chalk.red(formatTokens(result.removedTokens) + " tokens")} (${Math.round((result.removedTokens / result.originalTokens) * 100)}%)`);
 console.log(chalk.gray("\n ✓ Context compressed. Use /status to verify.\n"));
 break;
 }

 case "continue": {
 // Resume from breakpoint if available
 if (!session.checkpoint || !session.checkpoint.isInterrupted) {
 console.log(chalk.gray(" No interrupted task to continue. Send a new message to start a task."));
 return;
 }
 return "continue";
 }
 ...
 /continue Resume interrupted task from breakpoint
 ...
 /connect Connect a third-party API provider
